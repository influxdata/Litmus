...
NODE_TYPES = {
    ast.ClassDef: 'Test Class',
    ast.FunctionDef: 'Test Method'
    # ast.Module: 'Module'
}
...
def print_docstrings(source, module='<string>'):
    ...
    Output could be ordered by type, name.
    '''
    docstring=''
    ...
    #    key=lambda x: (x[1], NODE_TYPES.get(type(x[0]))))
    ...
    for type_, group in grouped:
        for node, name, lineno, docstring in group:
            ...
            heading="%s '%s'" % (type_, name)
            #heading = "%s '%s', line %s" % (type_, name, lineno or '?')
            if heading.find('footer') == -1 and heading.find('header') == -1 \
                    and heading.find('run_tests') == -1:
                print(heading)
                print('-' * len(heading))
                print(docstring or '')
                print('\n')
