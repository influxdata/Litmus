...
from src.cloud.rest_api.conftest import user_names
...
class TestGetAllUsersAPI(object):
    ...
    def test_get_all_users_count(self):
        ...
        self.mylog.info(test_name + 'Assert expected_count ' + str(expected_users_count) +
                        ' equals to actual count ' + str(actual_count))
        assert expected_users_count == actual_count, self.mylog.info(test_name + 'Assertion failed')
    ...
    def test_verify_created_users(self):
        ...
        for name in user_names:
            ...
            assert success, self.mylog.info(test_name + 'Assertion failed: user \'%s\' could not be found')
        self.footer(test_name)
